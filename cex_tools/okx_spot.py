...
from cex_tools.exchange_model.spot_position_model import OkxSpotDetail
...
class OkxSpot:
    ...
    def __init__(self, api_key="", secret_key="", passphrase="", erc20_deposit_addr="",
                 **kwargs):
        self.sub_account = kwargs.get("sub_account", "SpotFutureArb")
        ...
        self.account = self.okxSPOT.account
        self.market = self.okxSPOT.market
        self.trade = self.okxSPOT.trade
        self.market.get_exchangeInfos(uly="", expire_seconds=7 * 24 * 3600)
        # except:
        #     self.okxSWAP = None
        #     self.account = None
        #     self.market = None
        ...
        self.exchange_code = ExchangeEnum.OKX_SPOT
    ...
    def get_position(self, symbol):
        okx_position = self.account.get_balance(instId=self.convert_symbol(symbol))["data"]["details"]
        # availBal, ccy, eqUsd, maxLoan, interest
        return OkxSpotDetail(okx_position)

    def get_available_balance(self, asset="USDT"):
        get_balance = self.okxSPOT.account.get_balance(ccy=asset)
    ...
    def get_position_max_open_usd_amount(self, pair=None):
        avail_buy = self.account.api.get_max_avail_size(self.convert_symbol(pair),
                                                        "cross")["data"][0]["availBuy"]
        return float(avail_buy)
    ...
    def get_all_tick_price(self, symbol=None):
        if symbol:
            ...
            for d in datas:
                try:
                    d["name"] = d["instId"].replace("-", "")
                    d["midPx"] = (float(d["askPx"]) + float(d["bidPx"])) / 2
                    ret.append(d)
                except:
                    pass
            return ret
    ...
    def get_tick_price(self, symbol):
        r = self.market.get_ticker(instId=self.convert_symbol(symbol))
    ...
    def get_klines(self, symbol, interval="3m", limit=500):
    ...
    def make_new_order(self, symbol, side, order_type, quantity, price=None, msg="", **kwargs):
        ...
        if quantity == 0:
            ...
            return
        if side == TradeSide.BUY and order_type == "MARKET":
            # 当币币杠杆以市价买入时，指计价货币的数量。
            quantity *= self.get_tick_price(symbol)
        contract_quantity = self.convert_size_to_contract(symbol, quantity)
        if float(contract_quantity) == 0:
            logger.warning(f"⚠️ {self.exchange_code}下单数量为0: {symbol} {side} {order_type} {price} {quantity}")
            return
        if price is not None:
            price = self.convert_price(symbol, price)
        start_time = time.time()
        if kwargs.get("reduceOnly", None) is not None:
        ...
        content = f"⚠️ {self.exchange_code} {msg} 下单: {symbol} {side} {order_type} {price} {quantity} " \
    ...
    def get_history_order(self, pair):
        ret = self.trade.api.get_orders_history(instType="SPOT",
    ...
    def transfer_usd_asset_to_sub_account(self, amount,
        ...
        for ccy in token_list:
            for info in balance_list:
                if info["ccy"] == ccy:
                    available_balance = float(self.get_max_withdrawal(ccy))
                    balance = int(min(balance_to_transfer, available_balance))
                    if balance < 1:
    ...
    def wait_for_token(self, token, amount):
        # 等待资金到账 (至少30min) 需要检查 防止重复运行
        before_token_balance = self.get_available_balance()
        # 等待到账
        max_waiting_second = 30 * 60
        start_time = time.time()
        while True:
            after_token_balance = self.get_available_balance()
            if after_token_balance >= before_token_balance + amount - 2:
                break
            else:
        ...
        send_slack_message("✅ 存币到{} {} {} 用时: {:.2f}秒".format(self.exchange_code,
                                                                    amount, token,
    ...
    def get_borrow_interest_rate(self, symbol="USDT"):
        r = self.account.api.get_interest_rate(ccy=symbol)
        r = float(r["data"][0]["interestRate"])
        return r * 24 * 365

    def get_max_withdrawal(self, symbol="USDT"):
        r = self.account.api.get_max_withdrawal(ccy=symbol)
        for info in r["data"]:
            if info["ccy"] == symbol:
                return float(info["maxWdEx"])
        return 0
