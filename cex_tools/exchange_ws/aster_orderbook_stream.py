...
@Author      : Arson
@File Name   : aster_orderbook_stream
@Description : Aster交易所WebSocket订单簿流监听器（异步实现）
@Time        : 2025/10/4
"""
import asyncio
import json
...
class AsterOrderBookStreamAsync(OrderBookStream):
    """Aster交易所订单簿流监听器（异步实现，Binance兼容协议）"""
    ...
    def _get_stream_name(self, pair: str) -> str:
        ...
        :return: 流名称 (如 "btcusdt@depth@100ms")
        """
        # Binance格式：symbol@depth@100ms (100ms更新一次，减少消息量)
    ...
    async def _fetch_orderbook_snapshot(self, pair: str) -> Optional[dict]:
        ...
        """
        url = f"https://fapi.asterdex.com/fapi/v1/depth?symbol={pair.lower()}&limit=1000"
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status == 200:
                        data = await response.json()
                        logger.debug(f"[Aster] {pair} 获取快照成功，lastUpdateId={data.get('lastUpdateId')}")
                        return data
                    else:
                        logger.error(f"[Aster] {pair} 获取快照失败: HTTP {response.status}")
                        return None
        except Exception as e:
            logger.error(f"[Aster] {pair} 获取快照异常: {e}")
            return None
    ...
    async def _initialize_orderbook(self, pair: str):
        ...
        try:
            ...
            if not snapshot:
                logger.error(f"[Aster] {pair} 初始化失败：无法获取快照")
                return
            ...
            logger.debug(f"[Aster] {pair} 本地订单簿初始化完成，"
            ...
            logger.error(f"[Aster] {pair} 初始化订单簿异常: {e}")
            logger.exception(e)
    ...
    def _apply_depth_update(self, pair: str, event: dict):
        ...
        try:
            ...
            logger.error(f"[Aster] {pair} 应用增量更新异常: {e}")
            logger.exception(e)
    ...
    def _publish_orderbook(self, pair: str):
        ...
        try:
            ...
            logger.error(f"[Aster] {pair} 发布订单簿异常: {e}")
            logger.exception(e)
    ...
    async def _listen_websocket(self):
        ...
        # Aster WebSocket地址 (使用Binance兼容协议)
        ws_url = f"wss://fstream.asterdex.com/stream?streams={stream_names}"
        ...
        while self._running:
            try:
                if retry_count == 0:
                    logger.debug(f"[Aster] 连接到 WebSocket: {ws_url}")
                else:
                    logger.debug(f"[Aster] 重连WebSocket (第{retry_count}次)")

                async with websockets.connect(ws_url, ping_interval=20, ping_timeout=5) as websocket:
                    self._ws_connection = websocket
                    logger.debug(f"[Aster] WebSocket 连接成功")

                    # 重连成功，重置计数器
                    if retry_count > 0:
                        logger.debug(f"[Aster] 重连成功！")
                    retry_count = 0
                    ...
                    while self._running:
                        try:
                            ...
                            # Aster 返回格式: {"stream": "btcusdt@depth@100ms", "data": {...}}
                            if "data" in data and "stream" in data:
                                await self._handle_depth_update(data["stream"], data["data"])

                        except asyncio.TimeoutError:
                            logger.warning(f"[Aster] WebSocket 超时，发送 ping...")
                            await websocket.ping()
                            self._initialized.clear()
                        except websockets.exceptions.ConnectionClosed:
                            logger.warning(f"[Aster] WebSocket 连接关闭，准备重连...")
                            self._initialized.clear()
                            break
                        except Exception as e:
                            logger.error(f"[Aster] 处理消息异常: {e}")
                            logger.exception(e)
                            self._initialized.clear()
                ...
                if not self._running:
                    # 如果是主动停止，不记录错误
                    logger.debug(f"[Aster] WebSocket已主动停止")
                    return

                # 所有异常都尝试重连
                if "no pong" in str(e) or "ConnectionClosed" in str(type(e).__name__):
                    logger.warning(f"[Aster] WebSocket连接断开: {e}")
                else:
                    logger.error(f"[Aster] WebSocket 连接异常: {e}")
                    logger.exception(e)
                ...
                # 等待后重连
                if self._running:
                    logger.debug(f"[Aster] {retry_delay}秒后重连...")
                    await asyncio.sleep(retry_delay)

            finally:
                self._ws_connection = None

        logger.debug(f"[Aster] WebSocket监听线程退出")
    ...
    async def _handle_depth_update(self, stream: str, data: dict):
        ...
        try:
            ...
            logger.error(f"[Aster] 处理订单簿更新异常: {e}")
            logger.exception(e)

    async def start(self):
        """启动 WebSocket 连接"""
        if self._running:
            logger.warning(f"[Aster] WebSocket 已在运行")
            return

        if not self.orderbook_callbacks:
            logger.error(f"[Aster] 没有订阅任何交易对")
            return
        ...
        logger.debug(f"[Aster] WebSocket 已启动，订阅 {len(self.orderbook_callbacks)} 个交易对")
    ...
    async def stop(self):
        ...
        logger.debug(f"[Aster] WebSocket 已停止")


if __name__ == '__main__':
    """测试代码"""
    async def test_aster_orderbook_stream():
        # 创建流
    ...
    # 运行测试
    asyncio.run(test_aster_orderbook_stream())