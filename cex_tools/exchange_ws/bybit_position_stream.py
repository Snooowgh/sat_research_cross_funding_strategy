...
class BybitPositionWebSocket(PositionWebSocketStream):
        ...
        try:
            ...
            if topic is None and message.get("op") == "auth":
                ...
                if message.get("success") is True:
                    ...
                    logger.error(f"[{self.exchange_code}] 认证失败: {message.get('ret_msg')}")
                ...
                if message.get("success") is True:
                    ...
                    logger.error(f"[{self.exchange_code}] 仓位订阅失败: {message.get('ret_msg')}")
                ...
                logger.debug(f"[{self.exchange_code}] 未知消息: {topic}")
    ...
    def _convert_bybit_position(self, position_data: dict) -> BybitPositionDetail:
        ...
        try:
            ...
            return BybitPositionDetail(position_data, exchange_code=self.exchange_code)
        except Exception as e:
                ...
                async with websockets.connect(
                    ...
                    while self._running:
                        try:
                            ...
                            logger.debug(f"[{self.exchange_code}] WebSocket 超时")
