...
class OkxPositionWebSocket(PositionWebSocketStream):
    ...
    async def _send_login_message(self):
        ...
        try:
            ...
            logger.debug(f"[{self.exchange_name}] 发送登录消息")
        except Exception as e:
            logger.error(f"[{self.exchange_name}] 发送登录消息失败: {e}")
    ...
    async def _handle_message(self, message: dict):
        ...
        try:
            ...
            if event == "login":
                ...
                if code == "0":
                    logger.info(f"[{self.exchange_name}] 登录成功")
                    self._login_sent = True
                    # 登录成功后发送订阅消息
                    await self._send_subscription_message()
                else:
                    logger.error(f"[{self.exchange_name}] 登录失败: {message.get('msg')}")

            elif event == "subscribe":
                # 订阅响应
                channel = message.get("arg", {}).get("channel")
                if channel == "positions":
                    logger.info(f"[{self.exchange_name}] {channel} 订阅成功")
                else:
                    logger.error(f"[{self.exchange_name}] {channel} 订阅失败: {message.get('msg')}")
                ...
                # 错误消息
                logger.error(f"[{self.exchange_name}] 收到错误消息: {message}")

            else:
                logger.debug(f"[{self.exchange_name}] 未知消息类型: {message}")
    ...
    def _convert_okx_position(self, position_data: dict) -> OkxPositionDetail:
        ...
        try:
            ...
            return OkxPositionDetail(position_data, exchange_code=self.exchange_name)
        except Exception as e:
    ...
    async def _listen_websocket(self):
        ...
        while self._running:
            try:
                if retry_count == 0:
                    logger.debug(f"[{self.exchange_name}] 连接到私有数据流: {self.private_ws_url}")
                else:
