...
@Description : BinanceUnified仓位WebSocket流实现（使用binance_f WebSocket User Data Stream）
@Time        : 2025/10/15
"""
import asyncio
import json
...
class BinanceUnifiedPositionWebSocket(PositionWebSocketStream):
    """BinanceUnified仓位WebSocket流实现（使用binance_f WebSocket User Data Stream）"""
    ...
    def __init__(self, api_key: str = None, secret: str = None, sandbox: bool = False, **kwargs):
        ...
        configuration = ConfigurationRestAPI(api_key=self.api_key, api_secret=self.secret,
                                             base_path=DERIVATIVES_TRADING_PORTFOLIO_MARGIN_REST_API_PROD_URL)
        self.rest_client = DerivativesTradingPortfolioMargin(config_rest_api=configuration)

        configuration_ws_streams = ConfigurationWebSocketStreams(
            stream_url=DERIVATIVES_TRADING_PORTFOLIO_MARGIN_WS_STREAMS_PROD_URL
        )
        self.ws_client = DerivativesTradingPortfolioMargin(config_ws_streams=configuration_ws_streams)
        #
        # WebSocket配置
        self.base_ws_url = (
    ...
    async def _get_listen_key(self) -> Optional[str]:
        ...
        """
        listen_key = self.rest_client.rest_api.start_user_data_stream().data().listen_key
        ...
        while self._running and self.listen_key:
            try:
                ...
                if self._running and self.listen_key:
                    self.rest_client.rest_api.keepalive_user_data_stream()
    ...
    async def _refresh_listen_key(self):
        ...
        try:
            ...
            if self.listen_key:
                try:
                    self.rest_client.rest_api.close_user_data_stream()
                except:
                    pass
